This repo introduces a simplified version of lookahead decoding. The major differences between this work and the orignal one:
* The orignal logic collect n-grams through LLM inferences which leads to longer input sequence
* This work use a Trie structure as an in-context learner to collect n-grams from the input/output. It doesn't use LLM to guess possible n-grams.

The following is the benchmark result generated by the `run_mbench_tried.sh` script. The simplified logic's performance is better than the llama implement but a little bit worse than the orignal version.

```
# llama
AVERAGE THROUGHPUT1 34.97099487485063 AVERAGE THROUGHPUT2 34.381050116947776 STAT [5595.359179976101, 160, 62440, 1816.1167209148407]

# lade
AVERAGE THROUGHPUT1 52.08850116541747 AVERAGE THROUGHPUT2 52.72204874529382 STAT [8334.160186466795, 160, 61755, 1171.3315675258636]

# triede
AVERAGE THROUGHPUT1 51.618357067591816 AVERAGE THROUGHPUT2 47.19973984317365 STAT [8258.937130814691, 160, 61919, 1311.8504509925842]
```

(The following content is from the original readme)
